...
class TFormer_V2(nn.Module):
    def __init__(self, num_patches=16, dim=768, depth=3, heads=8, mlp_dim=1024, dim_head=64, dropout=0.0):
        super().__init__()
        self.dim = dim
        self.cls_token = nn.Parameter(torch.randn(1, 1, dim))
        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches+1, dim))
    ...
    def forward(self, x):
        x = x.contiguous().view(-1, 16, self.dim)
        b, n, _ = x.shape
        cls_tokens = repeat(self.cls_token, '() n d -> b n d', b=b)
        x = torch.cat((cls_tokens, x), dim=1)
        x = x + self.pos_embedding[:, :(n+1)]
...
def temporal_transformer(depth=6):
    return TFormer(depth=depth)
